...
Each article is tagged with multiple labels and has different attributes, ranging from the publish date, the author or the section where it was posted on the \textit{New York Times} website.
As labels we used the online sections a article has been posted to.
...
Since the extraction of concept maps for documents of this size takes a long time, we randomly selected 20 documents for each of the six labels, resulting in 120 articles.
...
\todo{Add paper reference: Daniele Vitale, Paolo Ferragina, and Ugo Scaiella. 2012. Classification of Short Texts by Deploying Topical Annotations.}

We obtained the copy from the authors website \footnote{\url{http://acube.di.unipi.it/repo/news.gz}}.
