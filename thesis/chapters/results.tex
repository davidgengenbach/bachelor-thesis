...
\begin{figure}[htb!]
	\centering
	{\includegraphics[width=0.9\linewidth]{assets/figures/percentage_degree.pdf}%
...
\answersummary{
The content-only graph kernel, which un-rolls the graph into text and then uses a conventional uni-gram \textit{BoW} approach, performs the best for both concept maps and co-occurrence graphs.
Removing the content, ie. the labels, from the graphs and then running WL results in classification scores close to the most-frequent dummy classifier.
When combining both, content and structure, the score is lower as content-only.
This all indicates that the content of the graphs has a greater importance than the structure since the structure gets discarded by the content-only approach and still performs the best.
}
...
The classification scores for this multi-word splitting nearly performs as well as the linearized, BoW approach from the previous question, Question \ref{question:importance_structure}.
...
While both \textit{L1} and \textit{L2} regularization discourage big weights, \textit{L1} penalizes small weights harder than \textit{L2}, leading to sparser models \cite{HonglakLeeAlexisBattleRajatRaina2006}.
