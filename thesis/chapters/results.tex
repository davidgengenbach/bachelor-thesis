...
\answersummary{
 	Using directed instead of un-directed edges most often results in significantly better performance when using the WL graph kernel.
 	A possible explanation for this better performance using directed edges is that neighborhood matches are far more easier since the neighborhood of a vertex is only defined through its outgoing edges, not the in-going as well.
...
\todo{Progression of first: analysis, then results}
