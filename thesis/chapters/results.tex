...
For this data, we use simple approaches for both graphs and texts: for the \textbf{texts} we vectorize the text documents with BoW with \textit{Tfidf}.
For \textit{graphs}, we vectorize the graphs with the plain Weisfeiler-Lehman graph kernel with $h = 4$ iterations.
...
\answersummary{
	...
	Applying dimensionality reduction, eg. \textit{PCA}, might be a solution to circumvent these issues.
