...
When looking at the most occurring edge labels per dataset, most of these most-frequent edge labels consist of stopwords or non-descriptive words like ``is", ``has", ``are".
...
For both, \textbf{(a)} and \textbf{(b)}, we use a graph kernel which first converts into a set of labels and then counts the number of these labels for each graph.
...
\todo{Examples of concept maps created from varying size texts}
\todo{Evaluate performance of varying size concept maps}
