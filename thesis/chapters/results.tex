...
\answersummary{
In our experiments, the text-only approach performs far better than the graph-only approaches, both concept maps and co-occurrence graphs.
}
...
\answersummary{
Splitting the multi-word labels shows the greatest performance improvement, ranging from a raise of 10\% to 24\% better F1 macro score over the normal un-split approach.
...
\answersummary{
		The bulk of all edge labels either occur only once per dataset or consist of stopwords.
...
For instance, the concept map creation for the texts in one of the smaller datasets, \textit{ling-dataset}, took over ??? hours and had a peak memory usage of ??? Megabyte\footnote{Machine specification. CPU: \textit{Intel(R) Xeon(R) CPU X5650 \@ 2.67GHz, 6-Core}, RAM: \textit{190GB}}.
...
\answersummary{
	...
	Applying dimensionality reduction might be a solution to circumvent these issues, yet it has to be noted that the feature vectors are very sparse, thus \textit{PCA} is not an option.
