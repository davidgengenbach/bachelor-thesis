...
\todo[inline]{Co-occurrence:"Text kernel" performs nearly as well as normal text}
\todo[inline]{Co-occurrence: only minor decrease (1\%) in performance when only using nouns}
\begin{figure}[ht]
\centering
\begin{tabular}{lrrr}
...
\todo[inline]{WL sparsity per dataset}
...
\todo[inline]{This is not true anymore. Classification performance on test set is different!}
...
\todo[inline]{SVM trained on combined, coefficients}
\todo[inline]{Analysis of coefficients}
...
\todo[inline]{Explain how to actually use the node weights.}
\todo[inline]{Remark that the node weights could be seen as an additional metric of frequency: nodes with a higher degree have been in the text more frequently.}
