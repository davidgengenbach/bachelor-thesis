...
def randomization_test(y_true, y_pred_a, y_pred_b, metric=sklearn.metrics.f1_score, num_trails=1000):
    metrics = np.empty((num_trails, 2), dtype=np.float64)
...
def plot_randomzation_test_distribution(result, metric=f1, metric_name='accuracy', num_trails = 1000):
    ...
    df.metric.plot(kind='hist', bins=100, ax=ax, title='Metric: {}, p={:.4f}'.format(metric_name, p))
    ax.axvline(diff, color='red')
    ax.axvline(-diff, color='red')
    ax.axvline(diffs.mean(), color='green')
    plt.show()
    plt.close(fig)
