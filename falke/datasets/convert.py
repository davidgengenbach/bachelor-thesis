...
def get_args():
    ...
    parser.add_argument('--in_file',
                        type=str,
                        help="See comment in this file",
                        default='20ng-stemmed.txt')
    parser.add_argument('--use_sklearn',
                        action='store_true')
...
def process(dataset_name, in_file, out_folder, train_size, random_state_for_shuffle, use_sklearn, one_document_per_folder, rename, max_elements):
    TOPIC_ID_OFFSET = 100
    data = get_topics_from_sklearn() if use_sklearn else get_topics(in_file)
    for idx, a in enumerate(data):
        data[idx] = (a[0], sanitize_text(a[1]))
    Y = [x[0] for x in data]
    X = [x[1] for x in data]
